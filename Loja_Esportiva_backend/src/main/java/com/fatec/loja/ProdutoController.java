...
public class ProdutoController {
    ...
    public List<Produto> listarVitrine(){
        List<Produto> retorno = bd.listarVitrine();
        return retorno.isEmpty() ? null : retorno;
    }

    @PostMapping("/api/produto/buscar")
    public List<Produto> fazerBusca(@RequestParam String busca){
        List<Produto> retorno = bd.fazerBusca(busca);
        return retorno.isEmpty() ? null : retorno;
    }
}
